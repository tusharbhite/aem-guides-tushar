...
public class PageReplicationEventHandler implements EventHandler {
    ...
    public void handleEvent(final Event event) {
        try {
            LOG.info("\n PageReplicationEventHandler Event Type : {} ", event.getTopic());
            props.put("replicationAction", ReplicationAction.fromEvent(event).getType());
            props.put("payload", ReplicationAction.fromEvent(event).getPath());

            jobManager.addJob("page/replication/job", props);
